...
2. Leadership Accountability Tracker — non-partisan scorecards for NASS, Governors, LGAs; monthly awards (Senator/Rep/Governor of the Month); right-of-reply mechanism
...
- Use data, structure, and measurable outcomes
    ...
    <>
      {parts.map((p, i) => {
        if (p.startsWith("**") && p.endsWith("**"))
          return <strong key={i} style={{ color: "#E2C068" }}>{p.slice(2, -2)}</strong>;
        if (p.startsWith("*") && p.endsWith("*"))
          return <em key={i} style={{ color: "#EDE8DA" }}>{p.slice(1, -1)}</em>;
        if (p.startsWith("`") && p.endsWith("`"))
          return <code key={i} style={{ background: "#050d05", color: "#C9A84C", padding: "1px 5px", borderRadius: 3, fontSize: "0.9em", fontFamily: "monospace" }}>{p.slice(1, -1)}</code>;
...
function Markdown({ text }) {
  ...
  while (i < lines.length) {
    const line = lines[i];
    if (line.startsWith("# ")) {
      elements.push(<h2 key={i} style={{ color: "#C9A84C", fontSize: 15, fontWeight: 700, fontFamily: "Georgia,serif", margin: "14px 0 5px", letterSpacing: 0.3 }}>{line.slice(2)}</h2>);
    } else if (line.startsWith("## ")) {
      elements.push(<h3 key={i} style={{ color: "#E2C068", fontSize: 13, fontWeight: 700, fontFamily: "Georgia,serif", margin: "12px 0 4px", borderBottom: "1px solid #C9A84C30", paddingBottom: 3 }}>{line.slice(3)}</h3>);
    } else if (line.startsWith("### ")) {
      elements.push(<h4 key={i} style={{ color: "#F5F1E8", fontSize: 11, fontWeight: 700, margin: "10px 0 3px", textTransform: "uppercase", letterSpacing: 0.8 }}>{line.slice(4)}</h4>);
    } else if (line.match(/^\|/)) {
      const rows = [];
      ...
      elements.push(
        <div key={`t${i}`} style={{ overflowX: "auto", margin: "10px 0" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
            <thead><tr>{rows[0]?.map((c, ci) => <th key={ci} style={{ background: "#050d05", color: "#C9A84C", padding: "6px 10px", textAlign: "left", fontSize: 10, fontWeight: 700, borderBottom: "2px solid #C9A84C" }}>{c}</th>)}</tr></thead>
            <tbody>{rows.slice(1).map((row, ri) => (<tr key={ri} style={{ background: ri % 2 === 0 ? "#1A3C1A25" : "transparent" }}>{row.map((c, ci) => <td key={ci} style={{ padding: "6px 10px", color: "#F5F1E8", fontSize: 11, borderBottom: "1px solid #1A3C1A50", verticalAlign: "top" }}><InlineText text={c} /></td>)}</tr>))}</tbody>
          </table>
        </div>
      );
      continue;
    } else if (line.startsWith("- ") || line.startsWith("• ")) {
      elements.push(<div key={i} style={{ display: "flex", gap: 8, margin: "3px 0", alignItems: "flex-start" }}><span style={{ color: "#C9A84C", flexShrink: 0, marginTop: 2 }}>▸</span><span style={{ color: "#F5F1E8", fontSize: 12, lineHeight: 1.65 }}><InlineText text={line.slice(2)} /></span></div>);
    } else if (/^\d+\.\s/.test(line)) {
      const num = line.match(/^(\d+)/)[1];
      elements.push(<div key={i} style={{ display: "flex", gap: 8, margin: "4px 0", alignItems: "flex-start" }}><span style={{ color: "#C9A84C", fontWeight: 700, fontSize: 11, minWidth: 18, flexShrink: 0 }}>{num}.</span><span style={{ color: "#F5F1E8", fontSize: 12, lineHeight: 1.65 }}><InlineText text={line.replace(/^\d+\.\s/, "")} /></span></div>);
    } else if (line.startsWith("---")) {
      elements.push(<div key={i} style={{ borderTop: "1px solid #C9A84C35", margin: "12px 0" }} />);
    } else if (line.trim() === "") {
      elements.push(<div key={i} style={{ height: 5 }} />);
    } else {
      elements.push(<p key={i} style={{ color: "#F5F1E8", fontSize: 12, lineHeight: 1.7, margin: "3px 0" }}><InlineText text={line} /></p>);
    }
    i++;
  }
...
export default function App() {
  ...
  const send = async (content) => {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({ model: "claude-sonnet-4-6", max_tokens: 1000, system: SYSTEM_PROMPT, messages: next.map(m => ({ role: m.role, content: m.content })) }),
      });
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", background: "linear-gradient(160deg,#122812 0%,#0a150a 45%,#050d05 100%)", fontFamily: "Georgia,serif", overflow: "hidden" }}>
      <style>{`*{box-sizing:border-box;margin:0;padding:0;}::-webkit-scrollbar{width:3px;}::-webkit-scrollbar-thumb{background:#C9A84C40;border-radius:2px;}@keyframes cb-pulse{0%,100%{opacity:.25;transform:scale(.75)}50%{opacity:1;transform:scale(1)}}textarea{outline:none;font-family:Georgia,serif;}textarea::placeholder{color:#6B7280;}.cb-qa:hover{background:#1A3C1A !important;border-color:#C9A84C70 !important;transform:translateY(-1px);}.cb-send:hover:not(:disabled){transform:scale(1.06);}.cb-act:hover{background:#C9A84C25 !important;}`}</style>
      <div style={{ background: "linear-gradient(90deg,#0a150a 0%,#1A3C1A 50%,#0a150a 100%)", borderBottom: "2px solid #C9A84C", padding: "0 18px", height: 54, display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 11 }}>
          <Logo size={36} />
          <div><div style={{ fontSize: 14, fontWeight: 700, color: "#C9A84C", letterSpacing: 0.4 }}>ISEYC Civic Brain</div><div style={{ fontSize: 8.5, color: "#E2C068", letterSpacing: 1.8, textTransform: "uppercase", marginTop: 1 }}>Institutional AI Intelligence System</div></div>
        </div>
        ...
        <>
          <div style={{ flex: 1, overflowY: "auto", padding: "18px 14px 0" }}>
            {messages.map((msg, i) => msg.role==="user" ? (
              ...
              <div key={i} style={{ display: "flex", gap: 9, marginBottom: 18, alignItems: "flex-start" }}>
                ...
                <div style={{ flex: 1, background: "linear-gradient(135deg,#0f1f0f 0%,#050d05 100%)", border: "1px solid #C9A84C28", borderRadius: "4px 14px 14px 14px", padding: "12px 15px", position: "relative" }}>
          ...
          <div style={{ padding: "10px 14px 14px", borderTop: "1px solid #C9A84C18", background: "#0a150a", flexShrink: 0 }}>
            <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
              ...
              <button className="cb-send" disabled={loading||!input.trim()} onClick={() => send(input)} style={{ width: 40, height: 40, borderRadius: 10, background: loading?"#1A3C1A60":"linear-gradient(135deg,#C9A84C,#E2C068)", border: "none", cursor: loading?"not-allowed":"pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "all 0.2s" }}>
      ...
      {tab === "dashboard" && (
        <div style={{ flex: 1, overflowY: "auto", padding: 14 }}>
          ...
          <div style={{ background: "linear-gradient(135deg,#0f1f0f,#050d05)", border: "1px solid #C9A84C25", borderRadius: 10, padding: 14, marginBottom: 12 }}>
            ...
            {PRIORITIES.map((p,i) => <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: i<PRIORITIES.length-1?8:0, paddingBottom: i<PRIORITIES.length-1?8:0, borderBottom: i<PRIORITIES.length-1?"1px solid #C9A84C10":"none" }}><div style={{ display: "flex", gap: 7, flex: 1, alignItems: "flex-start" }}><span style={{ color: "#C9A84C", flexShrink: 0, marginTop: 1, fontSize: 10 }}>▸</span><span style={{ fontSize: 11, color: "#F5F1E8", lineHeight: 1.4 }}>{p.task}</span></div><span style={{ fontSize: 9, color: "#C9A84C", background: "#C9A84C15", borderRadius: 4, padding: "2px 6px", flexShrink: 0, marginLeft: 8 }}>{p.due}</span></div>)}
          </div>
      ...
      {tab === "pillars" && (
        <div style={{ flex: 1, overflowY: "auto", padding: 14 }}>
          <p style={{ fontSize: 10, color: "#6B7280", marginBottom: 12 }}>Tap <strong style={{ color: "#C9A84C" }}>Activate →</strong> on any pillar to get its full Kaduna deployment plan.</p>
